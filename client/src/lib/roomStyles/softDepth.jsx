// Style: Soft Depth — layered translucent shapes and soft fills, no hard
// outlines. This is the project's original (and until Phase 10, only)
// rendering, reorganized behind the Phase 10 style-renderer interface (see
// ./index.js for the interface contract) without changing what it draws —
// every shape, coordinate, and colour below is unchanged from the
// pre-Phase-10 RoomArt.jsx, just relocated into the six interface hooks.
...
function drawFloor({ room, W, H, cell, finish, color, rand }) {
  ...
  if (room.type === 'balcony') {
    ...
    // building. Balconies are outdoor/non-indoor rooms, so drawWalls never
    // fires for them — this is their only edge treatment.
...
function drawPool({ room, W, H, finish, color, rand }) {
  const r = rand
...
function drawFurniture({ type, W, H, tier, finishKey }) {
  ...
  // always-drawn balustrade in drawFloor's main body, not furniture art.
