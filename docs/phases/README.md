...
- [x] **Phase 10 — Illustration style system.** Room rendering refactored into a pluggable style-renderer interface (`client/src/lib/roomStyles/`); the original rendering became the first style ("Soft Depth") and a complete second style ("Line Art" — bold outlines, material colour-coding) was built against the same interface. Always-visible live style switcher near the canvas, `style_changed` event. Closed 2026-08-05, see `docs/phases/phase-10.md`.
